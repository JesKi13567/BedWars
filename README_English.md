...
## Current version: beta1.5.1 for MC1.20.1
### Things have been achieved

1. ONLY support the overworld dimension.
1. Support Chinese(Simplified & Traditional) & English.
1. Support 64 players with 8 teams, each team has dynamic people counts!
1. In-game instructions.
1. There are 3 game types in this datapack (Classic, Exp & URF), with tips next to the Global Point.
1. Only support 1 map, forceload chunks near Global Point, radius 110m, and the map can be 「clean」 & 「convert」.
1. Use Spawn Eggs to place BedWars things.
1. In 「Operation Table」, you can set stage gaps, go to 「Container Chunk」 to custom item prices.
1. There is a Test Mode: Rebirth time & Item cooldown are reduced, unlock all alternations, shop items are free!
1. Players around the Global Point can modify (admins can block settings by placing blocks):
