...
# 掉落物处理
execute as @e[type=item, tag=!jkbw] unless data entity @s Item{id: "minecraft:arrow"} unless data entity @s Item{id: "minecraft:tipped_arrow"} unless data entity @s Item{tag: {jkbw: ["offhand_use"]}} run data merge entity @s {Item: {tag: {CanDestroy: ["#jkbw:candestroy"], CanPlaceOn: ["#jkbw:canplaceon"], HideFlags: 24}}, Tags: ["jkbw"]}
