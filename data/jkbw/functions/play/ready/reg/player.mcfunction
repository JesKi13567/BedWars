...
#execute if entity @e[limit=1,tag=jkbw_worldspawn] if score @s jkbw.Entity.Y <= @e[limit=1,tag=jkbw_worldspawn] jkbw.Entity.Y run tellraw @a ["<",{"selector":"@s"},"> 唔呃，逃不掉呢~"]
execute if entity @e[limit=1,tag=jkbw_worldspawn] if score @s jkbw.Entity.Y <= @e[limit=1,tag=jkbw_worldspawn] jkbw.Entity.Y run tp @s @e[limit=1,tag=jkbw_worldspawn]
