...
execute if score #ready_count1 jkbw.mem matches 0.. if score #ready_count2 jkbw.mem matches 0 run title @a times 0s 2s 0s
...
execute if score #ready_count1 jkbw.mem matches 5 if score #ready_count2 jkbw.mem matches 0 if score #teams jkbw.mem matches 3 if score #reg_num jkbw.mem matches 13.. run tellraw @a [{"text":"【⭐起床战争⭐】","color":"yellow"},{"text":"因人数达到上限，稍后将选取部分玩家旁观游戏！","color":"green"}]
...
execute if score #ready_count1 jkbw.mem matches ..-60 if score #ready_count2 jkbw.mem matches 0 run function jkbw:play/ready/reg/start_flag/ready