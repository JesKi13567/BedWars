...
execute if score #teams jkbw.mem matches 3 if entity @e[limit=1,tag=jkbw_start_check,scores={jkbw.mem=10}] run tag @e[limit=1,tag=jkbw_start_check] add jkbw_start_ready
execute if score #teams jkbw.mem matches 3 unless entity @e[limit=1,tag=jkbw_start_ready] run tellraw @a ["",{"text":"\n还没有达到游戏开始的条件！","color":"red"},{"text":"\n请检查以下几项是否成功设置：\n","color":"gold"},{"text":"1.全局传送点\n2.红队的床点、出生点、团队箱子点\n3.蓝队的床点、出生点、团队箱子点\n4.绿队的床点、出生点、团队箱子点\n","color":"yellow"},"请管理员输入/function #load重新调整！"]
