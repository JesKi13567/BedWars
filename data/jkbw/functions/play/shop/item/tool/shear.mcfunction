...
# 失败购买（不够物资）
execute unless score #exp_mode jkbw.mem matches 1 as @s[scores={jkbw.Player.HasShears=0},tag=!jkbw_buy_success] run scoreboard players operation #shop_temp jkbw.mem = @s jkbw.Player.OwnIrons
...
execute unless score #exp_mode jkbw.mem matches 1 run tellraw @s[scores={jkbw.Player.HasShears=0},tag=!jkbw_buy_success] [{"text":"无法购买","color":"red"},{"text":" 剪刀","color":"yellow","bold":true},"，你还需要 ",{"score":{"name": "#shop_temp","objective":"jkbw.mem"},"color":"yellow"}," 块铁锭！"]
...
execute unless score #exp_mode jkbw.mem matches 1 run clear @s[scores={jkbw.Player.HasShears=0},tag=jkbw_buy_success] iron_ingot 20
