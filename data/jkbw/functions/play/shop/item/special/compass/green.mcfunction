...
execute if score #shop_mode jkbw.mem matches 0 run tellraw @s[tag=!jkbw_bought,tag=!jkbw_buy_success] [{"text":"无法购买","color":"red"},{"text":" 绿队指示指南针 ","color":"yellow"},"，你还需要 ",{"score":{"name": "#shop_temp","objective":"jkbw.mem"},"color":"yellow"}," 块绿宝石！"]
execute if score #shop_mode jkbw.mem matches 1 run tellraw @s[tag=!jkbw_bought,tag=!jkbw_buy_success] [{"text":"无法购买","color":"red"},{"text":" 绿队指示指南针 ","color":"yellow"},"，你还需要 ",{"score":{"name": "#shop_temp","objective":"jkbw.mem"},"color":"yellow"}," 点经验！"]
playsound entity.villager.no player @s[tag=!jkbw_bought,tag=!jkbw_buy_success]

# 失败购买
tellraw @s[tag=jkbw_bought] [{"text":"你已经购买过","color":"red"},{"text":" 绿队指示指南针 ","color":"yellow"},"了！"]
...
tellraw @s[tag=!jkbw_bought,tag=jkbw_buy_success] [{"text":"成功购买","color":"green"},{"text":" 绿队指示指南针 ","color":"gold"},"！"]
