...
# 失败购买（不够物资）
execute if score #exp_mode jkbw.mem matches 0 as @s[tag=!jkbw_player_has_armor,tag=!jkbw_buy_success] run scoreboard players operation #shop_temp jkbw.mem = @s jkbw.Player.OwnEmeralds
...
execute if score #exp_mode jkbw.mem matches 0 as @s[tag=!jkbw_player_has_armor,tag=!jkbw_buy_success] run scoreboard players remove #shop_temp jkbw.mem 6
...
execute if score #exp_mode jkbw.mem matches 0 run tellraw @s[tag=!jkbw_player_has_armor,tag=!jkbw_buy_success] [{"text":"无法购买","color":"red"},{"text":" 钻石套装","color":"yellow","bold":true},"，你还需要 ",{"score":{"name": "#shop_temp","objective":"jkbw.mem"},"color":"yellow"}," 块绿宝石！"]
...
execute if score #exp_mode jkbw.mem matches 0 run clear @s[tag=jkbw_buy_success] emerald 6
