...
execute if score #time jkbw.mem matches 601.. as @s[nbt=!{EnderItems: [{tag: {jkbw: ["team", "dragon"]}}]}] run function jkbw:play/shop/team/update/dragon
execute if score #time jkbw.mem matches ..600 as @s[nbt=!{EnderItems: [{tag: {jkbw: ["team", "dragon"]}}]}] run tellraw @s {"text":"死斗模式已开始，无法再购买！","color":"red"}
