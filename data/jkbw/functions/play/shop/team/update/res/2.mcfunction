...
execute unless score #multi_mode jkbw.mem matches 1 unless score #exp_mode jkbw.mem matches 1 as @s[tag=!jkbw_buy_success] run scoreboard players remove #shop_temp jkbw.mem 12
execute if score #multi_mode jkbw.mem matches 1 unless score #exp_mode jkbw.mem matches 1 as @s[tag=!jkbw_buy_success] run scoreboard players remove #shop_temp jkbw.mem 6
...
execute unless score #exp_mode jkbw.mem matches 1 run tellraw @s[tag=!jkbw_buy_success] [{"text":"无法购买","color":"red"},{"text":" 资源点升级 III","color":"yellow","bold":true},"，你还需要 ",{"score":{"name": "#shop_temp","objective":"jkbw.mem"},"color":"yellow"}," 块钻石！"]
...
execute unless score #multi_mode jkbw.mem matches 1 unless score #exp_mode jkbw.mem matches 1 as @s[tag=jkbw_buy_success] run clear @s[tag=jkbw_buy_success] diamond 12
execute if score #multi_mode jkbw.mem matches 1 unless score #exp_mode jkbw.mem matches 1 as @s[tag=jkbw_buy_success] run clear @s[tag=jkbw_buy_success] diamond 6
