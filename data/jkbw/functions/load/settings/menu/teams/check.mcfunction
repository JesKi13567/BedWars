...
execute if score #teams jkbw.mem matches 3.. as @e[limit=1,tag=jkbw_bed_green] at @s if block ~ ~ ~ green_bed run tellraw @a[tag=jkbw_admin] {"text":"绿队床安放成功！","color":"green"}
...
execute if score #teams jkbw.mem matches 3.. as @e[limit=1,tag=jkbw_bed_green] at @s unless block ~ ~ ~ green_bed run tellraw @a[tag=jkbw_admin] "绿队床不在标记位置上！"
