...
execute if score #loaded jkbw.mem matches 1 if score #state jkbw.mem matches 1.. run tellraw @a[tag=jkbw_admin] [{storage: "jk:bw", nbt: "txt.print.bedwars", color: "yellow"}, " ", {storage: "jk:bw", nbt: "txt.print.is_running", color: "yellow"}, "\n", {text: "============== ", color: "green"}, {text: "[", color: "red"}, {storage: "jk:bw", nbt: "txt.print.stop_game", color: "red", click_event: {action: "run_command", command: "/trigger jkbw.admin.trigger set 98"}}, {text: "]", color: "red"}, {text: " ==============", color: "green"}]
