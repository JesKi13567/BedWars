# 团队升级——检查升级
$execute if entity @e[type=text_display, tag=jkbw_spawn_$(team), scores={jkbw.Team.$(item)=2}, limit=1] run tellraw @s [{storage: "jk:bw", nbt: "txt.play.shop.buy.cannot", color: "red"}, " ", {storage: "jk:bw", nbt: "txt.shop.team.$(item).name", color: "yellow"}, ", ", {storage: "jk:bw", nbt: "txt.play.becuz.lvl_max.self"}]
