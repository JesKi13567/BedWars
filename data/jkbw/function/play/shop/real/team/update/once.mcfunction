...
# 失败购买
$execute unless score #1 jkbw.mem matches $(id) run tellraw @s[tag=jkbw_bought] [{storage: "jk:bw", nbt: "txt.play.shop.buy.cannot", color: "red"}, " ", {storage: "jk:bw", nbt: "txt.shop.team.$(item).name", color: "yellow"}, ", ", {storage: "jk:bw", nbt: "txt.play.becuz.lvl_max.team.own"}]
$execute if score #1 jkbw.mem matches $(id) if entity @e[type=text_display, tag=jkbw_spawn_$(team), limit=1, scores={jkbw.Team.$(item)=1}] run tellraw @s[tag=jkbw_bought] [{storage: "jk:bw", nbt: "txt.play.shop.buy.cannot", color: "red"}, " ", {storage: "jk:bw", nbt: "txt.shop.team.$(item).name", color: "yellow"}, ", ", {storage: "jk:bw", nbt: "txt.play.becuz.lvl_max.team.own"}]
...
$execute if score #res_mode jkbw.mem matches 0 run tellraw @s[tag=!jkbw_bought, tag=!jkbw_buy_success] [{storage: "jk:bw", nbt: "txt.play.shop.buy.cannot", color: "red"}, " ", {storage: "jk:bw", nbt: "txt.shop.team.$(item).name", color: "yellow"}, ", ", {storage: "jk:bw", nbt: "txt.play.shop.you_need"}, " ", {translate: "item.minecraft.diamond"}, "*", {score: {name: "#shop_temp", objective: "jkbw.mem"}, color: "yellow"}, "!"]
$execute if score #res_mode jkbw.mem matches 1 run tellraw @s[tag=!jkbw_bought, tag=!jkbw_buy_success] [{storage: "jk:bw", nbt: "txt.play.shop.buy.cannot", color: "red"}, " ", {storage: "jk:bw", nbt: "txt.shop.team.$(item).name", color: "yellow"}, ", ", {storage: "jk:bw", nbt: "txt.play.shop.you_need"}, " ", {storage: "jk:bw", nbt: "txt.global.lvl"}, "*", {score: {name: "#shop_temp", objective: "jkbw.mem"}, color: "yellow"}, "!"]
