$execute as @s[scores={jkbw.Player.Levels.$(tool)=4}] run tellraw @s [{storage: "jk:bw", nbt: "txt.play.shop.buy.cannot", color: "red"}, " ", {storage: "jk:bw", nbt: "txt.shop.item.$(tool)", color: "yellow"}, " ", {storage: "jk:bw", nbt: "txt.global.update", color: "yellow"}, ", ", {storage: "jk:bw", nbt: "txt.play.becuz.lvl_max.self"}]
