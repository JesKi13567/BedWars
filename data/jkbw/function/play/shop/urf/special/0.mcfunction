...
# 成功购买
$execute if score #1 jkbw.mem matches $(id) run give @s[tag=jkbw_buy_success] mooshroom_spawn_egg[custom_data={jkbw: ["tnt"]}, can_break={predicates: [{blocks: "#jkbw:candestroy"}], show_in_tooltip: false}, can_place_on={predicates: [{blocks: "#jkbw:canplaceon"}], show_in_tooltip: false}, entity_data={id: "tnt", Tags:["jkbw", "jkbw_tnt", "jkbw_new_entity"], fuse:10000s}, custom_name='{"translate": "block.minecraft.tnt", "color": "white", "italic": false}', max_stack_size=64]
...
$execute if score #2 jkbw.mem matches $(id) run give @s[tag=jkbw_buy_success] fire_charge[can_break={predicates: [{blocks: "#jkbw:candestroy"}], show_in_tooltip: false}, food={can_always_eat: true, eat_seconds: 100000, nutrition: 0, saturation: 0}, max_stack_size=64]
