...
# 成功购买
$execute if score #1 jkbw.mem matches $(id) run give @s[tag=jkbw_buy_success] $(item)[can_break={predicates: [{blocks: "#jkbw:candestroy"}], show_in_tooltip: false}, can_place_on={predicates: [{blocks: "#jkbw:canplaceon"}], show_in_tooltip: false}, max_stack_size=99]
$execute if score #2 jkbw.mem matches $(id) run give @s[tag=jkbw_buy_success] $(item)[max_stack_size=99]
