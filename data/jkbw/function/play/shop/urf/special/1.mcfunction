...
$execute if score #4 jkbw.mem matches $(id) run give @s[tag=jkbw_buy_success] ice[can_break={predicates: [{blocks: "#jkbw:candestroy"}], show_in_tooltip: false}, can_place_on={predicates: [{blocks: "#jkbw:canplaceon"}], show_in_tooltip: false}, food={can_always_eat: true, eat_seconds: 100000, nutrition: 0, saturation: 0}]
