...
$execute if score @s jkbw.Player.Own.xpLevelsReal >= #XPsword_$(material) jkbw.mem run tag @s add jkbw_buy_success
...
$execute if score #res_mode jkbw.mem matches 1 as @s[tag=!jkbw_buy_success] run scoreboard players operation #shop_temp jkbw.mem -= #XPsword_$(material) jkbw.mem
...
$execute unless score #ENABLE_shield jkbw.mem matches 2 run give @s[tag=jkbw_buy_success] $(material)_sword[tooltip_display={hidden_components: [can_break]}, can_break={blocks: "#jkbw:candestroy"}, unbreakable={}]
$execute if score #ENABLE_shield jkbw.mem matches 2 run give @s[tag=jkbw_buy_success] $(material)_sword[tooltip_display={hidden_components: [can_break]}, can_break={blocks: "#jkbw:candestroy"}, unbreakable={}, blocks_attacks={block_sound: {sound_id: "entity.player.hurt"}, damage_reductions: [{base: 1, factor: 0.5}], disable_cooldown_scale: 0}]
...
$execute if score #res_mode jkbw.mem matches 1 run scoreboard players operation @s[tag=jkbw_buy_success] jkbw.Player.Own.xpLevelsReal -= #XPsword_$(material) jkbw.mem
