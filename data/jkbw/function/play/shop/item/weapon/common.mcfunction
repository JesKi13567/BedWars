...
# 成功购买
$execute if score #1 jkbw.mem matches $(id) run give @s[tag=jkbw_buy_success] stick[can_break={predicates: [{blocks: "#jkbw:candestroy"}], show_in_tooltip: false}, can_place_on={predicates: [{blocks: "#jkbw:canplaceon"}], show_in_tooltip: false}, enchantments={knockback: 1}, max_stack_size=64]
...
$execute if score #3 jkbw.mem matches $(id) run give @s[tag=jkbw_buy_success] arrow[max_stack_size=64] 8
...
$execute if score #5 jkbw.mem matches $(id) run give @s[tag=jkbw_buy_success] tipped_arrow[potion_contents="slowness", max_stack_size=64] 3
