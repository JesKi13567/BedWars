...
# 成功购买
$execute if score #1 jkbw.mem matches $(id) run give @s[tag=jkbw_buy_success] stick[can_break={predicates: [{blocks: "#jkbw:candestroy"}], show_in_tooltip: false}, can_place_on={predicates: [{blocks: "#jkbw:canplaceon"}], show_in_tooltip: false}, enchantments={knockback: 1}, max_stack_size=99]
$execute if score #2 jkbw.mem matches $(id) run give @s[tag=jkbw_buy_success] shield[can_break={predicates: [{blocks: "#jkbw:candestroy"}], show_in_tooltip: false}, can_place_on={predicates: [{blocks: "#jkbw:canplaceon"}], show_in_tooltip: false}, damage=326]
$execute if score #3 jkbw.mem matches $(id) run give @s[tag=jkbw_buy_success] arrow[max_stack_size=99] 8
$tellraw @s[tag=jkbw_buy_success] [{"storage": "jk:bw", "nbt": "txt.print.buy_ok", "color": "green"}, " ", {"translate": "item.minecraft.$(item)", "color": "gold"}, "!"]
$execute if score #4 jkbw.mem matches $(id) run give @s[tag=jkbw_buy_success] trident[can_break={predicates: [{blocks: "#jkbw:candestroy"}], show_in_tooltip: false}, can_place_on={predicates: [{blocks: "#jkbw:canplaceon"}], show_in_tooltip: false}, damage=248]
$execute if score #5 jkbw.mem matches $(id) run give @s[tag=jkbw_buy_success] tipped_arrow[potion_contents="slowness", max_stack_size=99] 3
$execute if score #6 jkbw.mem matches $(id) run give @s[tag=jkbw_buy_success] mace[can_break={predicates: [{blocks: "#jkbw:candestroy"}], show_in_tooltip: false}, can_place_on={predicates: [{blocks: "#jkbw:canplaceon"}], show_in_tooltip: false}, damage=499]
