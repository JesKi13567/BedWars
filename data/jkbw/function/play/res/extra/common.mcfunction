...
$execute as @s[scores={jkbw.ResCount.$(res)=$(count)..}] run summon item ~ ~ ~ {Age: -32768, PickupDelay: 18, Invulnerable: True, Item: {id: "$(res)", count: $(count)}, Tags: ["jkbw_extra_$(res)", "jkbw_res"]}
