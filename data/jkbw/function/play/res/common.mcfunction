...
$execute if score #summon_$(res) jkbw.mem matches 1 at @e[type=text_display, tag=jkbw_$(res)_time] unless entity @e[type=item, tag=jkbw_extra_$(res), dx=0, dy=-8, dz=0] run summon item ~ ~ ~ {Age: -32768, PickupDelay: 18, Invulnerable: True, Item: {id: "$(res)", count: 1, components: {can_break: {predicates: [{blocks: "#jkbw:candestroy"}], show_in_tooltip: false}, can_place_on: {predicates: [{blocks: "#jkbw:canplaceon"}], show_in_tooltip: false}}}, Tags: ["jkbw", "jkbw_res", "jkbw_res_$(res)"]}
