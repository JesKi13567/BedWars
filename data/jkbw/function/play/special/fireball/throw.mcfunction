...
# 差储存为速度向量
execute store result entity @e[tag=jkbw_throwing, limit=1] Motion[0] double 0.004 run scoreboard players operation #x2 jkbw.mem -= #x1 jkbw.mem
execute store result entity @e[tag=jkbw_throwing, limit=1] Motion[1] double 0.004 run scoreboard players operation #y2 jkbw.mem -= #y1 jkbw.mem
execute store result entity @e[tag=jkbw_throwing, limit=1] Motion[2] double 0.004 run scoreboard players operation #z2 jkbw.mem -= #z1 jkbw.mem
# 清理
