tag @s add jkbw_this
execute as @a[scores={jkbw.Player.ID=1..}] if score @s jkbw.Team.ID = @n[type=marker, tag=jkbw_this] jkbw.Team.ID run tellraw @s {storage: "jk:bw", nbt: "txt.play.enderchest.break", color: "red"}
kill @s
