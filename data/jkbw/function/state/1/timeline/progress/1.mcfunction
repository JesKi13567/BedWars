...
execute if score #res_mode jkbw.mem matches 2 run tellraw @a ["", {translate: "attribute.name.max_health", color: "aqua"}, " ", {storage: "jk:bw", interpret: true, nbt: "txt.play.now_become"}, " ", {text: "30", color: "red"}, {storage: "jk:bw", interpret: true, nbt: "txt.global.char.dot"}]
execute if score #ENABLE.glider jkbw.mem matches 1 run tellraw @a [{storage: "jk:bw", interpret: true, nbt: "txt.ready.control_panel.glider_mode.name", color: "gold"}, {storage: "jk:bw", interpret: true, nbt: "txt.global.lock.uned"}, "\n", {storage: "jk:bw", interpret: true, nbt: "txt.ready.control_panel.glider_mode.p0"}]
