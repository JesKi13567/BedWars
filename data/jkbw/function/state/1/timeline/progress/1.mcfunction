...
execute unless score #exp_mode jkbw.mem matches 6 run bossbar set jkbw:game_progress color green
execute unless score #exp_mode jkbw.mem matches 4 run tellraw @a ["", {translate: "item.minecraft.diamond", color: "aqua"}, " ", {storage: "jk:bw", nbt: "txt.play.now_become"}, " ", {storage: "jk:bw", nbt: "txt.global.tier"}, " ", {text: "II", color: "yellow"}, {storage: "jk:bw", nbt: "txt.global.char.dot"}]
