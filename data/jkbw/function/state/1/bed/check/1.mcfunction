...
# 团灭后提示，队伍箱子解锁与淘汰（一次）
$execute if score $$(team) jkbw.Bed.State matches 3 if score $$(team) jkbw.Team.Alive matches 0 unless score #exp_mode jkbw.int matches 6 run tellraw @a ["\n", {storage: "jk:bw", interpret: true, nbt: "txt.color.$(team)", color: "$(color)", bold: true}, {storage: "jk:bw", interpret: true, nbt: "txt.global.team", color: "$(color)", bold: true}, {storage: "jk:bw", interpret: true, nbt: "txt.play.eliminated"}, "\n"]
$execute if score $$(team) jkbw.Bed.State matches 3 if score $$(team) jkbw.Team.Alive matches 0 if score #exp_mode jkbw.int matches 6 run tellraw @a ["\n", {storage: "jk:bw", interpret: true, nbt: "txt.color.$(team)", color: "white", bold: true}, {storage: "jk:bw", interpret: true, nbt: "txt.global.team", color: "white", bold: true}, {storage: "jk:bw", interpret: true, nbt: "txt.play.eliminated"}, "\n"]
