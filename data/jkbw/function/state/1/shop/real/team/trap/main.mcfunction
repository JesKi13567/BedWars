# 检测能否买
$function jkbw:state/1/shop/real/team/trap/check {team: '$(team)'}

# 失败购买
$tellraw @s[tag=jkbw_bought] [{storage: "jk:bw", nbt: "txt.play.shop.buy.cannot", color: "red"}, " ", {storage: "jk:bw", nbt: "txt.shop.trap.list.$(item).name", color: "yellow"}, {storage: "jk:bw", nbt: "txt.global.char.comma"}, {storage: "jk:bw", nbt: "txt.play.becuz.lvl_max.team.trap"}]
...
# 成功购买
$execute as @s[tag=!jkbw_bought, tag=jkbw_buy_success] run function jkbw:state/1/shop/real/team/trap/clear {team: '$(team)', item: $(item)}
