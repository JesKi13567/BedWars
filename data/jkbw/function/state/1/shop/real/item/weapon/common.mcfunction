# 检测能否买
$execute if score @s jkbw.Player.Own.gold_ingot >= #$(item) jkbw.int run tag @s add jkbw_buy_success
...
$execute if score #res_mode jkbw.int matches 0 run tellraw @s[tag=!jkbw_buy_success] [{storage: "jk:bw", interpret: true, nbt: "txt.play.shop.buy.cannot", color: "red"}, " ", {translate: "item.minecraft.$(item)", color: "yellow"}, {storage: "jk:bw", interpret: true, nbt: "txt.global.char.comma"}, {storage: "jk:bw", interpret: true, nbt: "txt.play.shop.you_need"}, " ", {translate: "item.minecraft.gold_ingot"}, "*", {score: {name: "#shop_temp", objective: "jkbw.int"}, color: "yellow"}, {storage: "jk:bw", interpret: true, nbt: "txt.global.char.exclamation"}]
...
$execute if score #2 jkbw.int matches $(id) run give @s[tag=jkbw_buy_success] shield[tooltip_display={hidden_components: [can_break]}, can_break={blocks: "#jkbw:candestroy"}]
$execute if score #3 jkbw.int matches $(id) run give @s[tag=jkbw_buy_success] arrow[tooltip_display={hidden_components: [can_break]}, can_break={blocks: "#jkbw:candestroy"}] 8
$execute if score #4 jkbw.int matches $(id) run give @s[tag=jkbw_buy_success] trident[tooltip_display={hidden_components: [can_break]}, can_break={blocks: "#jkbw:candestroy"}, enchantments={loyalty: 3}, unbreakable={}]
$execute if score #5 jkbw.int matches $(id) run give @s[tag=jkbw_buy_success] tipped_arrow[tooltip_display={hidden_components: [can_break]}, can_break={blocks: "#jkbw:candestroy"}, potion_contents="slowness"] 3
$execute if score #6 jkbw.int matches $(id) run give @s[tag=jkbw_buy_success] mace[tooltip_display={hidden_components: [can_break]}, can_break={blocks: "#jkbw:candestroy"}, unbreakable={}]
...
$execute if score #res_mode jkbw.int matches 0 run scoreboard players operation @s[tag=jkbw_buy_success] jkbw.Player.Own.gold_ingot -= #$(item) jkbw.int
