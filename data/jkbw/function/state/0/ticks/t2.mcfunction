...
# 地图标记
execute as @n[type=marker, tag=jkbw_map_point, scores={jkbw.int=1..}] at @s run function jkbw:state/0/panel/contents/map/point/real

# 轮换检查点（地图清理完后触发）
execute if score #map_rotation_check jkbw.int matches 2 unless entity @e[type=marker, tag=jkbw_map_point] run function jkbw:state/0/panel/contents/map/rotation/real
