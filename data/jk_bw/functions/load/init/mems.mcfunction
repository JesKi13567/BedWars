# 这里存放一些全局分数
#  loaded       play_mode        teams      gaming       feedback
# 是否加载   经典模式/经验模式    队伍数量   正在进行游戏    函数反馈
#scoreboard players set #loaded jk_bw_mem 1
scoreboard players set #play_mode jk_bw_mem 0
scoreboard players set #teams jk_bw_mem 2
scoreboard players set #gaming jk_bw_mem 0
scoreboard players set #feedback jk_bw_mem 0
#  test_mode   current_game
#   测试模式    当前游戏场次
scoreboard players reset #test_mode jk_bw_mem
scoreboard players reset #current_game jk_bw_mem

# 计算用数字
scoreboard players set #-1 jk_bw_mem -1
scoreboard players set #1 jk_bw_mem 1
scoreboard players set #2 jk_bw_mem 2
scoreboard players set #3 jk_bw_mem 3
scoreboard players set #4 jk_bw_mem 4
scoreboard players set #5 jk_bw_mem 5
scoreboard players set #6 jk_bw_mem 6
scoreboard players set #7 jk_bw_mem 7
scoreboard players set #8 jk_bw_mem 8
scoreboard players set #10 jk_bw_mem 10
scoreboard players set #12 jk_bw_mem 12
scoreboard players set #16 jk_bw_mem 16
scoreboard players set #20 jk_bw_mem 20
scoreboard players set #24 jk_bw_mem 24
scoreboard players set #30 jk_bw_mem 30
scoreboard players set #32 jk_bw_mem 32
scoreboard players set #40 jk_bw_mem 40
scoreboard players set #100 jk_bw_mem 100
scoreboard players set #120 jk_bw_mem 120
scoreboard players set #127 jk_bw_mem 127
scoreboard players set #300 jk_bw_mem 300
scoreboard players set #600 jk_bw_mem 600