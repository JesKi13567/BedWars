...
# 差储存为速度向量
execute store result entity @e[tag=jk_bw_throwing,limit=1] power[0] double 0.004 run scoreboard players operation #x2 jk_bw_mem -= #x1 jk_bw_mem
execute store result entity @e[tag=jk_bw_throwing,limit=1] power[1] double 0.004 run scoreboard players operation #y2 jk_bw_mem -= #y1 jk_bw_mem
execute store result entity @e[tag=jk_bw_throwing,limit=1] power[2] double 0.004 run scoreboard players operation #z2 jk_bw_mem -= #z1 jk_bw_mem
# 清理
