...
execute as @s[tag=!jk_bw_team_has_sth,tag=!jk_bw_buy_success] run scoreboard players operation #shop_temp jk_bw_mem -= #5 jk_bw_mem
...
tellraw @s[tag=!jk_bw_team_has_sth,tag=!jk_bw_buy_success] [{"text":"无法购买","color":"red"},{"text":" 死斗模式增强","color":"yellow","bold":true},"，你还需要 ",{"score":{"name": "#shop_temp","objective":"jk_bw_mem"},"color":"yellow"}," 块钻石！"]
...
clear @s[tag=!jk_bw_team_has_sth,tag=jk_bw_buy_success] diamond 5
