...
execute unless score #multi_mode jk_bw_mem matches 1 as @s[tag=!jk_bw_buy_success] run scoreboard players operation #shop_temp jk_bw_mem -= #8 jk_bw_mem
...
tellraw @s[tag=!jk_bw_buy_success] [{"text":"无法购买","color":"red"},{"text":" 资源点升级 II","color":"yellow","bold":true},"，你还需要 ",{"score":{"name": "#shop_temp","objective":"jk_bw_mem"},"color":"yellow"}," 块钻石！"]
...
execute unless score #multi_mode jk_bw_mem matches 1 as @s[tag=!jk_bw_buy_success] run clear @s[tag=jk_bw_buy_success] diamond 8
