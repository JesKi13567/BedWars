...
execute unless score #exp_mode jk_bw_mem matches 1 run tellraw @s[tag=!jk_bw_buy_success] [{"text":"无法购买","color":"red"},{"text":" 速度 II 药水","color":"yellow","bold":true},"，你还需要 ",{"text":"1","color":"yellow"}," 块绿宝石！"]
execute if score #exp_mode jk_bw_mem matches 1 run tellraw @s[tag=!jk_bw_buy_success] [{"text":"无法购买","color":"red"},{"text":" 速度 II 药水","color":"yellow","bold":true},"，你还需要 ",{"score":{"name": "#shop_temp","objective":"jk_bw_mem"},"color":"yellow"}," 点经验！"]
playsound entity.villager.no player @s[tag=!jk_bw_buy_success]

# 成功购买
give @s[tag=jk_bw_buy_success] potion{CustomPotionColor:702679,display:{Name:'{"text":"速度 II 药水（45 秒）","color":"aqua","italic":false}'},Potion:"water",CustomPotionEffects:[{Id:1,Amplifier:1,Duration:900,ShowParticles:false}],CanPlaceOn:["#jk_bw:canplaceon"],CanDestroy:["#jk_bw:candestroy"],HideFlags:28}
tellraw @s[tag=jk_bw_buy_success] [{"text":"成功购买","color":"green"},{"text":" 速度 II 药水","color":"gold","bold":true},"！"]
