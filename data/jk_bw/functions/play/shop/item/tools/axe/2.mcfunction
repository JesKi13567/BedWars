...
tellraw @s[tag=!jk_bw_buy_success] [{"text":"无法升级为","color":"red"},{"text":" 石斧","color":"yellow","bold":true},"，你还需要 ",{"score":{"name": "#shop_temp","objective":"jk_bw_mem"},"color":"yellow"}," 块铁锭！"]
playsound entity.villager.no player @s[tag=!jk_bw_buy_success]

# 成功购买
tellraw @s[tag=jk_bw_buy_success] [{"text":"成功升级为","color":"green"},{"text":" 石斧","color":"gold","bold":true},"！"]
