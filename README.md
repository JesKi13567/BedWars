...
2. 若为单人存档，此时才进入（若已进入则重进）。
若为服务器，此时才开启（若已开启则重启）。
3. 使用 `/reload` 或 `/function #load` 呼出菜单。
4. 注：仅支持主世界维度。
...
|小人国|玩家的尺寸变为原来的 1/2。|
|幸运之床|商店和资源点禁用。每经过 5 秒，玩家将免费获得随机的商品（不含团队升级）。|
...
2. For singleplayer: Enter the world now (re‑enter if already loaded).
For servers: Start the server now (restart if already running).
3. Type `/reload` or `/function #load` to open the menu.
4. Note: This datapack only support the overworld dimension.
...
2. Support max 8 teams & 64 players.
3. Place items using spawn eggs.
4. All non-durable items have a maximum stack size of 64.
5. There is a Test Mode, available for solo play and free shopping.
6. Additional features are listed separately below.
...
|-|-|
|Disabled| \ |
|Blitz|All Resource Points are at max level, Obsidian/Ender Chest cannot be bought, Ender Pearl has lower price, has auto-bridging.|
|No Jumps|Players cannot jump, but can step up 3 blocks.|
|Lilliput|Set players' Scale to 1/2.|
|Beds of Fortune|Shops and Resource Points are banned. For every 5 seconds, players will get random shop loots freely (team updates not included).|
|Patpat|Set both players' Entity & Block Interaction Range to 32.|
|Color Blind|All players have the same white name tags, armor, and blocks.|
|Armed|All bows are replaced with guns.|
|Zero Friction|All players have no friction.|
